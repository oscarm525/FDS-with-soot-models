...
IMPLICIT NONE
PRIVATE
CHARACTER(255), PARAMETER :: divgid='$Id: divg.f90 20330 2014-08-25 20:58:33Z drjfloyd@gmail.com $'
CHARACTER(255), PARAMETER :: divgrev='$Revision: 20330 $'
CHARACTER(255), PARAMETER :: divgdate='$Date: 2014-08-25 15:58:33 -0500 (lun 25 de ago de 2014) $'
...
SPECIES_LOOP: DO N=1,N_TRACKED_SPECIES
   ...
   !$OMP PARALLEL DO PRIVATE(DZDX, DZDY, DZDZ) SCHEDULE (STATIC)
      DO K=0,KBAR
         DO J=0,JBAR
            DO I=0,IBAR
               IF (CAMBIO .AND. SPECIES_MIXTURE(N)%ID=='SOOT' .AND. DNS) THEN
			      IF (PREDICTOR) THEN
			      UU=>U
			      VV=>V
			      WW=>W
                  ...
                  ZZZ(1:4) = ZZP(I-1:I+2,J,K,N)/TMP(I-1:I+2,J,K)
		          RHO_D_DZDX(I,J,K)= SCALAR_FACE_VALUE(UU(I,J,K),ZZZ,FLUX_LIMITER) !Ys/T EN FACE	X	       
		          RHO_D_DZDX(I,J,K)=RHO_D_DZDX(I,J,K)*0.0556*MU(I,J,K)*DTDX
                  DTDY = (TMP(I,J+1,K)-TMP(I,J,K))*RDYN(J)
                  ZZZ(1:4) = ZZP(I,J-1:J+2,K,N)/TMP(I,J-1:J+2,K)
		          RHO_D_DZDY(I,J,K)= SCALAR_FACE_VALUE(VV(I,J,K),ZZZ,FLUX_LIMITER) !Ys/T EN FACE	Y	       
		          RHO_D_DZDY(I,J,K)=RHO_D_DZDY(I,J,K)*0.0556*MU(I,J,K)*DTDY
                  DTDZ = (TMP(I,J,K+1)-TMP(I,J,K))*RDZN(K)
                  ZZZ(1:4) = ZZP(I,J,K-1:K+2,N)/TMP(I,J,K-1:K+2)
                  RHO_D_DZDZ(I,J,K)= SCALAR_FACE_VALUE(WW(I,J,K),ZZZ,FLUX_LIMITER) !Ys/T EN FACE	Z	       
		          RHO_D_DZDZ(I,J,K)=RHO_D_DZDZ(I,J,K)*0.0556*MU(I,J,K)*DTDZ
               ELSE
