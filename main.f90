...
CHARACTER(255), PARAMETER :: mainid='$Id: main.f90 20560 2014-09-26 12:37:27Z randy.mcdermott $'
CHARACTER(255), PARAMETER :: mainrev='$Revision: 20560 $'
CHARACTER(255), PARAMETER :: maindate='$Date: 2014-09-26 07:37:27 -0500 (vie 26 de sep de 2014) $'
...
IF (CAMBIO .AND. REAC_SOURCE_CHECK .AND. CYLINDRICAL) CALL DAR_REAC_SOURCE
IF (CYLINDRICAL) CALL DAR_OTRASV
CALL END_FDS
...
SUBROUTINE DAR_REAC_SOURCE
USE GLOBAL_CONSTANTS
INTEGER :: N,I,J,K,YLIM=1
REAL :: POS_R(1:IBAR), POS_Z(1:KBAR)

IF(RLIM==1 .OR. ZLIM==1) RETURN

IF(RLIM>IBAR) RLIM=IBAR
IF(ZLIM>KBAR) ZLIM=KBAR

OPEN(UNIT=88,FILE=TRIM(CHID)//'_reac.dat')
...
WRITE(88,*) POS_R(1)
 DO I=2,RLIM
    POS_R(I)=POS_R(I-1)+DX(I-1)
    WRITE(88,*) POS_R(I)
ENDDO
 
WRITE(88,*) POS_Z(1)
 DO K=2,ZLIM
   POS_Z(K)=POS_Z(K-1)+DZ(K-1)
   WRITE(88,*) POS_Z(K)
ENDDO
DO N=1,N_TRACKED_SPECIES
   DO K=1,ZLIM
      DO J=1,YLIM
         DO I=1,RLIM
            WRITE(88,*) REAC_SOURCE_TERM(I,J,K,N)
         ENDDO
      ENDDO
   ENDDO
ENDDO
!~ DO N=1,N_TRACKED_SPECIES
!~    WRITE(88,*) SPECIES_MIXTURE(N)%ID
!~ ENDDO
!~ CLOSE(UNIT=88)

END SUBROUTINE DAR_REAC_SOURCE

SUBROUTINE DAR_OTRASV
USE GLOBAL_CONSTANTS
INTEGER :: N,I,J,K,YLIM=1
REAL :: POS_R(1:IBAR), POS_Z(1:KBAR)
...
OPEN(UNIT=88,FILE=TRIM(CHID)//'_otrasv.dat')
...
ENDDO
   ...
   DO K=1,ZLIM
      DO J=1,YLIM
         DO I=1,RLIM
		    WRITE(88,*) RHO(I,J,K)
!~             WRITE(88,*) (ZZ(I,J,K,REACTION(1)%FUEL_SMIX_INDEX)*REACTION(1)%S-ZZ(I,J,K,2)+0.233)+(REACTION(1)%S+0.233)
!~             WRITE(88,*) (ZZ(I,J,K,REACTION(1)%FUEL_SMIX_INDEX)*REACTION(1)%S-0.231164*ZZ(I,J,K,REACTION(1)%AIR_SMIX_INDEX)+0.231164)+(REACTION(1)%S+0.231164) !para air y no o2
!~             ZZ(I,J,K,REACTION(1)%FUEL_SMIX_INDEX)+ZZ(I,J,K,Z_INDEX)* &
