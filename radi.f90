...
CHARACTER(255), PARAMETER :: radiid='$Id: radi.f90 20330 2014-08-25 20:58:33Z drjfloyd@gmail.com $'
CHARACTER(255), PARAMETER :: radirev='$Revision: 20330 $'
CHARACTER(255), PARAMETER :: radidate='$Date: 2014-08-25 15:58:33 -0500 (lun 25 de ago de 2014) $'
   ...
   BUILD_KAPPA_ARRAY: IF (N_RADCAL_SPECIES>0) THEN
      ...
      BAND_LOOP_Z: DO IBND = 1,NSB
         ...
         T_LOOP_Z: DO K = 0,N_KAPPA_T
            ...
            IF (ABS(RCT-RADTMP)<=0.4_EB*(RTMPMAX-RTMPMIN)/N_KAPPA_T .AND. PATH_LENGTH > 0.0_EB) THEN
               RCT_SKIP = K
               CYCLE T_LOOP_Z
            ENDIF
...
IF ( MOD(RAD_CALL_COUNTER,TIME_STEP_INCREMENT)==1 .OR. TIME_STEP_INCREMENT==1 .OR. INITIALIZATION_PHASE .OR. ICYC==1) THEN
...
BAND_LOOP: DO IBND = 1,NUMBER_SPECTRAL_BANDS
   ...
   IF (NLP>0 .AND. VIRTUAL_PARTICLES) THEN
      DO IP = 1,NLP
         ...
         SELECT CASE(SF%GEOMETRY)
            CASE(SURF_CARTESIAN)
               SURFACE_AREA = 2._EB*SF%LENGTH*SF%WIDTH  
         ...
         DO K=1,KBAR
            DO J=1,JBAR
               DO I=1,IBAR
                  ...
                  KAPPA(I,J,K) = KAPPA(I,J,K) + GET_KAPPA(ZZ_GET,TYY,IBND,RHO(I,J,K),TMP(I,J,K))
               ENDDO
            ENDDO
         ENDDO
         !$OMP END DO
   ...
   INTENSITY_UPDATE: IF (UPDATE_INTENSITY) THEN
      ...
      N_UPDATES = 1
      IF (RAD_CALL_COUNTER==1 .OR. INITIALIZATION_PHASE .OR. ICYC==1) N_UPDATES = ANGLE_INCREMENT
...
FUNCTION GET_KAPPA(Z_IN,TYY,IBND,RHO,TMP)

! Returns the radiative absorption

USE PHYSICAL_FUNCTIONS, ONLY : GET_MASS_FRACTION_ALL,GET_MOLECULAR_WEIGHT
REAL(EB), INTENT(INOUT) :: Z_IN(0:N_TRACKED_SPECIES),RHO,TMP
REAL(EB) :: KAPPA_TEMP,INT_FAC,GET_KAPPA,SCALED_Y_RADCAL_SPECIES,MWA,KAPPA_S
...
ENDDO
   KAPPA_S=0
   IF (CAMBIO) KAPPA_S=1226*(Z_IN(SOOT_INDEX)*RHO/SPECIES(SOOT_INDEX)%DENSITY_SOLID)*TMP
   GET_KAPPA=GET_KAPPA+KAPPA_S
